...
static volatile uint8_t serialHeadRX[UART_NUMBER],serialTailRX[UART_NUMBER]; //UART_NUMBER = 1;
...
void store_uart_in_buf(uint8_t data, uint8_t portnum) {
  #if defined(SPEKTRUM) //Not Defined
    if (portnum == SPEK_SERIAL_PORT) {
      if (!spekFrameFlags) { 
        sei();
        uint32_t spekTimeNow = timer0_overflow_count; //Move timer0_overflow_count into registers so we don't touch a volatile twice
        ...
        if (spekInterval > 5) {  //Potential start of a Spektrum frame, they arrive every 11 or every 22 ms. Mark it, and clear the buffer. 
...
#if defined(PROMINI) //defined for us
  ISR(USART_RX_vect)  { 
...
uint8_t SerialRead(uint8_t port) {
  #if defined(PROMICRO)//not defined. PROMINI defined for us. -Alex
    #if defined(TEENSY20)
