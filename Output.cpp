...
#if defined(SERVO) //servo is not defined
...
#if defined(PROMINI) //THIS IS WHAT WE ARE USING
...
#if defined(PROMINI)
#if defined(EXT_MOTOR_32KHZ) //What we are using
...
#if defined(PROMINI)
  //TCCR1A/B: Timer/counter control registers. WGM11: Wavefrom generation mode.
  TCCR1A = (1 << WGM11); // phase correct mode & no prescaler
  TCCR1B = (1 << WGM13) | (1 << CS10);
#if defined(EXT_MOTOR_32KHZ)
  ICR1   = 0x00FF; // TOP to 255; Input Capture Register
  TCCR2B =  (1 << CS20);
#elif defined(EXT_MOTOR_4KHZ)
...
void mixTable() {
  ...
  return;
#endif
#define PIDMIX(X,Y,Z) rcCommand[THROTTLE] + axisPID[ROLL]*X + axisPID[PITCH]*Y + YAW_DIRECTION * axisPID[YAW]*Z //I AM HERE
...
#define CAM_TIME_LOW  conf.servoConf[2].middle
  ...
  for (i = 0; i < NUMBER_MOTOR; i++) {
    ...
    if ((rcData[THROTTLE] < MINCHECK) && !f.BARO_MODE) //MINCHECK = 1100
#ifndef MOTOR_STOP
