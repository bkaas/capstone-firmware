...
/**************************************************************************************/
#if defined(STANDARD_RX) //NOT DEFINED
...
ISR(PCINT0_vect) {
  ...
  void rxInt(void) {
    ...
    if (diff > 3000) chan = 0;
     ...
     rcValue[chan] = diff;
    //   for(int j=0;j<70;j++){
         rcValue[0] =1100;// conf.throttleIn; //added by Daigle Jan 3
    //    i=i+10;
...
/*
      if (900 < diff && diff < 2200 && chan == 0 && f.ARMED) { //Only if the signal is between these values it is valid, otherwise the failsafe counter should move up
        rcValue[chan] = conf.throttleIn;
#if defined(FAILSAFE)
        if (chan < 4 && diff > FAILSAFE_DETECT_TRESHOLD) GoodPulses |= (1 << chan); // if signal is valid - mark channel as OK
        if (GoodPulses == 0x0F) {                                            // If first four chanells have good pulses, clear FailSafe counter
          GoodPulses = 0;
...
#define SBUS_SYNCBYTE 0x0F // Not 100% sure: at the beginning of coding it was 0xF0 !!!
    ...
    while (SerialAvailable(SBUS_SERIAL_PORT)) {
      ...
      if (sbusIndex == 25) {
        ...
        if (!((sbus[23] >> 3) & 0x0001))
        {
          if (failsafeCnt > 20) failsafeCnt -= 20;  // clear FailSafe counter
          else failsafeCnt = 0;
        }
#endif
      }
    ...
    if (spekFrameFlags == 0x01) {   //The interrupt handler saw at least one valid frame start since we were last here.
      if (SerialAvailable(SPEK_SERIAL_PORT) == SPEK_FRAME_SIZE) {  //A complete frame? If not, we'll catch it next time we are called.
        ...
        for (uint8_t b = 2; b < SPEK_FRAME_SIZE; b += 2) {
          ...
          //        if ((spekChannel < RC_CHANS) && (spekChannel != THROTTLEPIN )) rcValue[spekChannel] = 988 + ((((uint16_t)(bh & SPEK_CHAN_MASK) << 8) + bl) SPEK_DATA_SHIFT);
          //        else if ((spekChannel <RC_CHANS) && (spekChannel = THROTTLEPIN)) rcValue[spekChannel] = 1400;
    ...
    for (chan = 0; chan < RC_CHANS; chan++) {
      if (chan < 8 && rcSerialCount > 0) { // rcData comes from MSP and overrides RX Data until rcSerialCount reaches 0
        ...
        if (rcSerial[chan] > 900) {
          rcData[chan] = rcSerial[chan]; // only relevant channels are overridden
        }
      }
    }
  }
