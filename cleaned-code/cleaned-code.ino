...
void loop()  {
  ...
  if ( infrared && dist > minDist) {
    tmpPitch = runPitchIR();
    tmpRoll = runRollIR();
    if (tmpPitch > 0) Serial.print("p" + String(tmpPitch));
    if (tmpRoll > 0) Serial.print("r" + String(tmpRoll));
  }
 ...
 * 1) Setting pinModes and defaults for adjustable parameters. 
 * 2) Communcation with Ultrasonic sensor.
 * 3) Send to PID black box.
 * 4) Main loop blocks (ultrasonic, IR, Bluetooth).
 */


// 1. Defaults for adjustable parameters.
void loadDefaults() {
  ...
  compFrac = 0.75;  //fraction of dip to compensate 'g'
}


// 2. Ultrasonic main loop block.
int runUltra() {
  ...
  state[2] = 0; //!digitalRead(irPinS);
    ...
    if((state[0]-state[2]) && go){
      ...
      return (midVal[0] + adjustment*(state[2] - state[0]));
       ...
       timeout =2;
       go = 1;
       return (midVal[0] - (int)(compFrac*direc));
     }
  ...
  state[3] = 0;//!digitalRead(irPinW); 
    ...
    if((state[1]-state[3]) && go1){
      ...
      return (midVal[1] + adjustment*(state[3] - state[1]));
       ...
       timeout1 =2;
       go1 = 1;
       return (midVal[1] - (int)(compFrac*direc1));
     }
...
// 4. Setup for serial/software serials.
void loadSerials() {
  Serial.begin(115200);
...
// 4. pinMode setup for IR sensors.
void initializePins() {
...
// 4. Communication with Ultrasonic sensor.
